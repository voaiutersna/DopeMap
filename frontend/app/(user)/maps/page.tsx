...
const toggleEnroll = async (mapId: string) => {
  ...
  if (!map.enrolled) {
    // enrolling
    const toastId = toast.info("Enrolling in roadmap...");

    try {
      const res = await api.post("/history", { roadmap_id: mapId });
      if (res.data.success) {
        setMaps((prev) =>
          prev.map((m) => (m.id === mapId ? { ...m, enrolled: true } : m))
        );
        toast.update(toastId, {
          render: "Successfully enrolled!",
          type: "success",
        });
      } else {
        toast.update(toastId, {
          render: "Failed to enroll.",
          type: "error",
        });
      }
    } catch (e) {
      ...
      toast.update(toastId, {
        render: "An error occurred while enrolling.",
        type: "error",
      });
    }
  } else {
    // unenrolling
    const toastId = toast.info("Unenrolling from roadmap...");
    ...
    try {
      ...
      if (record) {
        ...
        if (res.data.success) {
          setMaps((prev) =>
            prev.map((m) => (m.id === mapId ? { ...m, enrolled: false } : m))
          );
          toast.update(toastId, {
            render: "Successfully unenrolled!",
            type: "success",
          });
        } else {
          toast.update(toastId, {
            render: "Failed to unenroll.",
            type: "error",
          });
        }
      } else {
        toast.update(toastId, {
          render: "No enrollment record found.",
          type: "warning",
        });
      }
    } catch (e) {
      ...
      toast.update(toastId, {
        render: "An error occurred while unenrolling.",
        type: "error",
      });
    }
  }
      ...
      <div className="bg-[#2f3131] min-h-[calc(100vh-104px)] text-zinc-200 font-mono flex flex-col items-center">
        <div className="container w-full py-8 sm:py-12 px-4 sm:px-6 flex flex-col gap-6 sm:gap-8">
          ...
          {loading ? (
            ...
            <div className="text-zinc-400 text-center text-sm sm:text-base">
              No maps found.
            </div>
          ) : (
